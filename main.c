...
int main(int argc, char *argv[]) {
    ...
    else if(argc==5)
    {
        ...
        unsigned long long nbr_ligne_p=line_matrix/world_size;
        double *matrix,*matrix1,*sub_matrix=(double*) malloc(sizeof(double)*line_matrix*column_matrix);

        if(rank==0)
        {
            double *result= malloc(sizeof(double)*column_matrix*line_matrix_1);
            matrix= declare_matrix(line_matrix,column_matrix);
            matrix1= declare_matrix_column(line_matrix,column_matrix);
        ...
        if(line_matrix%world_size!=0)
        {
            ...
            if(rank==world_size-1)
            {
                nbr_ligne_p=nbr_ligne_p+(line_matrix%world_size);
            }

        }
        double *result_p=(double*) malloc(sizeof(double)*nbr_ligne_p);
        for (int i = 0; i < column_matrix_1; ++i)
        {
            MPI_Bcast(&matrix1[i*line_matrix_1],column_matrix_1,MPI_DOUBLE,0,MPI_COMM_WORLD);
            for (int j = 0; j < nbr_ligne_p; ++j) {
                result_p[j]=0;
                for (int k = 0; k < column_matrix; ++k)
                {
                    result_p[j]=result_p[j]+matrix[j*column_matrix+k]*matrix1[i*line_matrix_1+k];
                }
            }
        }
